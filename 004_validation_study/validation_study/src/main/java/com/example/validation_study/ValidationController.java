...
public class ValidationController {
    ...
    @PostMapping
    //ResponseEntity는 전에 했다.
    //Valid : 검증해라는 의미이다. 검증한 후에 BindingResult result 에서 결과를 받는다.
    //@RequestBody :
    //이 때 파라미터에서 왼쪽에 객체(m), 오른쪽에는 결과(result) 가 있어야 한다.
    public ResponseEntity<?> addMember(@Valid @RequestBody Member m, BindingResult result) {
        // 에러가 발생했는지 여부 확인
        if(result.hasErrors()) {
            // 발생한 에러 개수 출력
            System.out.println("error count : " + result.getErrorCount());
            //getFieldErrors() 검증을 했을 때 에러들이 들어오는 객체
...
// HashMap은 Jackson 메시지 컨버터가 JSON으로 변환 가능
    //List 안에 있는 FieldError 값이 코드에서 에러가 난 부분들
    //이를 map으로 바꿔준다.
